    ...
    try {
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            if (!user.UserProfile) {
                ...
                console.log(`사용자 ${user.email}의 UserProfile 생성 완료 (student_id: ${studentId})`);
            } else {
                // 기존 UserProfile 업데이트
                if (!user.UserProfile.student_id) {
                    ...
                    console.log(`사용자 ${user.email}의 student_id 업데이트 완료 (student_id: ${studentId})`);
                } else {
                    console.log(`사용자 ${user.email}의 UserProfile 이미 존재 (student_id: ${user.UserProfile.student_id})`);
                }
            }
        }
